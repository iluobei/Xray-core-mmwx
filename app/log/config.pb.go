...
type Config struct {
	...
	// 不写访问日志的入站 tag。空 = 一个都不排除。
	// 默认值在 infra/conf 那层决定(未配置 → ["api"]),这里拿到的已经是最终清单。
	AccessExcludeInboundTags []string `protobuf:"bytes,8,rep,name=access_exclude_inbound_tags,json=accessExcludeInboundTags,proto3" json:"access_exclude_inbound_tags,omitempty"`
...
func (x *Config) GetAccessExcludeInboundTags() []string {
	if x != nil {
		return x.AccessExcludeInboundTags
	}
	return nil
}
...
const file_app_log_config_proto_rawDesc = "" +
	"\n" +
	"\x14app/log/config.proto\x12\fxray.app.log\x1a\x14common/log/log.proto\"\x9d\x03\n" +
	"\x06Config\x12;\n" +
	...
	"\fmask_address\x18\a \x01(\tR\vmaskAddress\x12=\n" +
	"\x1baccess_exclude_inbound_tags\x18\b \x03(\tR\x18accessExcludeInboundTags*5\n" +
	"\aLogType\x12\b\n" +
