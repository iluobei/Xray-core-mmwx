...
type DeviceConfig struct {
	...
	// 每个 peer 一个 User,携带 email —— 限速、并发连接数与 per-user 流量统计全靠它。
	// 上游 345c76f9 用的是 field 5,但本 fork 的 5 已被 num_workers 占用,故改用 10。
	Users         []*protocol.User `protobuf:"bytes,10,rep,name=users,proto3" json:"users,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
const file_proxy_wireguard_config_proto_rawDesc = "" +
	...
	"\rno_kernel_tun\x18\t \x01(\bR\vnoKernelTun\x120\n" +
	"\x05users\x18\n" +
	" \x03(\v2\x1a.xray.common.protocol.UserR\x05users\"\\\n" +
...
var file_proxy_wireguard_config_proto_depIdxs = []int32{
	...
	3, // 2: xray.proxy.wireguard.DeviceConfig.users:type_name -> xray.common.protocol.User
